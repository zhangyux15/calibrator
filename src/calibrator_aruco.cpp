...
bool ArucoInternalCalibrator::Push(const cv::Mat& img, const bool& display)
{
	...
	if (!markerCorners.empty()) {
		cv::aruco::interpolateCornersCharuco(markerCorners, markerIds, img, charucoboard, currentCharucoCorners,
			currentCharucoIds);

		if (currentCharucoCorners.total() > 4) {
			allCharucoCorners.emplace_back(currentCharucoCorners);
			allCharucoIds.emplace_back(currentCharucoIds);
		}
	}

	if (display && currentCharucoCorners.total()) {
		cv::Mat temp;
		const float rate = 1024.f / float(img.cols);
		cv::resize(img, temp, cv::Size(1024, int(std::ceil(float(img.rows) * rate))));
		cv::Mat cornersClip = currentCharucoCorners.clone();
		cornersClip = cornersClip * rate;
		cv::aruco::drawDetectedCornersCharuco(temp, cornersClip, currentCharucoIds);
		cv::imshow("aruco", temp);
		cv::waitKey(1);
	}
	return !markerCorners.empty();
}


void ArucoInternalCalibrator::Calib()
{
	...
	// calibrate
	cv::aruco::calibrateCameraCharuco(allCharucoCorners, allCharucoIds, charucoboard,
	...
	// evaluate
	//for (int idx = 0; idx < allCharucoCorners.size(); idx++) 
	//	std::cout << "err: " << perViewErrors(idx) << "pixel" << std::endl;
